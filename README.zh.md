...
`spend()` 返回 `today`、`month`、`allTime`(各含金额、按价格档位的拆分、请求数、未计价 token),外加 `since`(保留的最早一行时间)、`currency`、`pricesAsOf`、`timezone`。
...
| `currency` | `[CNY, USD]` | 账户持有多种货币余额时的优先顺序。 |
...
这些数字由你维护。内置默认值于 **2026-08-23** 从 <https://api-docs.deepseek.com/quick_start/pricing> 抄录;DeepSeek 改价时,更新表并改 `asOf`——浮层会把它显示为「价格数据日期」。运行时不会去抓那个页面。

schema 里没有任何厂商专有词汇。一个条目写出自己的基准价,和一组有序的具名档位;每个档位在该条目自己的 IANA 时区里声明若干时间窗,并且要么重写价格,要么给基准价乘一个系数。请求时刻落入的第一个档位生效;不落入任何窗口时用基准价。

```yaml
prices:
  asOf: '2026-08-23'        # 界面显示为「价格数据日期」
  currency: USD             # ISO 4217;所有费率与合计都以它计价
  entries:
    - model: deepseek-v4-flash
      provider: deepseek-official   # 可选;省略则该模型在任意路由上都用这条
      per: 1000000                  # 一个费率单位覆盖多少 token
      timezone: UTC                 # 下面时间窗所用的时区
      baseName: off-peak            # 基准档在界面上的名字(默认 "standard")
      base:
        input: 0.22                 # 未命中缓存的输入 token
        inputCacheHit: 0.007        # 命中缓存的输入 token
        output: 0.66                # 生成 token
...
- `timezone` 必须是本运行时认识的 IANA 时区;`per` 必须为正;费率不得为负;同一 provider 下同一模型只能出现一次。

DeepSeek 公布的是「谷时价为峰时价的一半;峰时为 UTC 周一至周五 01:00–04:00 与 06:00–10:00」。上面把它写成 `base` 放谷时价 + `peak` 档位乘 2:峰时是两段连续的工作日区间,谷时则是它们零散的补集,这样写更短,算出来的费率完全一样。DeepSeek 没有单独给缓存写入和思考 token 定价,所以这两项留空取默认:**缓存写入按未命中输入价计费,思考 token 按输出价计费。**

一个带周末档的三档服务方示例——**此例为虚构,仅作说明**:

```yaml
    - model: example-large            # 并非真实服务方,也不是真实价格
      per: 1000000
...
改动价格表会改变会话投影的缓存版本,于是每个会话下次读取时按新价重算,而不是在旧价算出的合计上继续累加。
...
- **内容** 每次 LLM 请求一行 JSON:时间、会话 id 与日志序号、模型与 provider id、五个 token 桶、费用、货币、生效的价格档位。未计价的行带 `"unpriced": true` 且费用为 0。**不含 prompt、不含回复、不含密钥、不含端点。**
...
```sh
dsh --profile <name> --dump-config   # 应出现 "# == @haoran/dsh-balance" 层
dsh web --profile <name>
```

用 `dsh plugin --profile <name> remove @haoran/dsh-balance` 卸载,依赖与层一起移除。账目文件会原样留下;要丢弃就删掉 `$DSH_HOME/dsh-balance/`。
