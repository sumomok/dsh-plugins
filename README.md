...
`spend()` answers `today`, `month`, and `allTime` — each with a cost, a per-tier split, a request count, and unpriced tokens — plus `since` (the oldest retained ledger row), `currency`, `pricesAsOf`, and `timezone`.
...
| `currency` | `[CNY, USD]` | Currency codes in descending preference, when the account holds several balances. |
...
| `prices` | DeepSeek's published rates | The price table. See below. |
...
The numbers are yours to maintain. The shipped default was transcribed on **2026-08-23** from <https://api-docs.deepseek.com/quick_start/pricing>; when DeepSeek changes a rate, update the table and bump `asOf`, which the popover shows as "prices as of". Nothing here fetches that page at runtime.
...
```yaml
prices:
  asOf: '2026-08-23'        # shown in the UI as "prices as of"
  currency: USD             # ISO 4217; every rate and every total is quoted in it
  entries:
    - model: deepseek-v4-flash
      provider: deepseek-official   # optional; omit to price the model on any route
      per: 1000000                  # tokens one rate unit covers
      timezone: UTC                 # the zone the windows below are written in
      baseName: off-peak            # what to call the base tier in the UI (default "standard")
      base:
        input: 0.22                 # cache-miss input tokens
        inputCacheHit: 0.007        # cache-hit input tokens
        output: 0.66                # generated tokens
...
- `timezone` must be an IANA zone this runtime knows; `per` must be positive; no rate may be negative; a model may appear once per provider.

DeepSeek publishes "off-peak rates are half of the peak rates, peak is 01:00–04:00 and 06:00–10:00 UTC Monday through Friday". That is written above as off-peak numbers in `base` plus a `peak` schedule that doubles them, because the peak windows are two contiguous weekday spans while the off-peak hours are their scattered complement. The resulting rates are identical either way. DeepSeek prices neither cache writes nor reasoning tokens separately, so both are left to their defaults: **a cache write bills at the cache-miss input rate, and a reasoning token bills at the output rate.**

A three-tier provider with a weekend rate — **this example is fictional, for illustration only**:

```yaml
    - model: example-large            # NOT a real provider or real prices
      per: 1000000
...
Editing the price table changes the per-session projection's cache version, so every session is re-priced on next read rather than continuing to add to totals computed at the old rates.
...
- **Contents** one JSON object per line, per LLM request: the time, the session id and log sequence number, the model and provider ids, the five token buckets, the cost, the currency, and the price tier that applied. Unpriced rows carry `"unpriced": true` and a zero cost. **No prompts, no completions, no keys, no endpoints.**
...
# or, once published:
dsh plugin --profile <name> add @haoran/dsh-balance
```

The package declares `dsh.bundle`, so `dsh plugin` appends its patch layer to the profile automatically. Verify before booting:

```sh
dsh --profile <name> --dump-config   # shows a "# == @haoran/dsh-balance" layer
dsh web --profile <name>
```

Remove it with `dsh plugin --profile <name> remove @haoran/dsh-balance`, which drops both the dependency and the layer. The ledger file is left where it is; delete `$DSH_HOME/dsh-balance/` to discard it.
