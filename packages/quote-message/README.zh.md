...
发出去的引用不会以一串 `>` 留在气泡内部。插件把它提到消息上方，排成一段纯排版的引文：左侧 2px 的竖线加次级字色的正文，没有卡片底也没有抬头标签，右对齐，宽度贴着文字并沿用气泡自己的上限。超过三行折叠，并给出 `展开` / `收起`；这个按钮只在文字真的溢出时出现，展开状态属于当前视图，不做持久化。模型读的那行 `> 引用：` 是标记引文的记号，本身不属于引文，因此它不进入显示，也永远不会从日志里被拿掉。
